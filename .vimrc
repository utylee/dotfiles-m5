...
" nmap <leader>ew :!tmux send-keys -t 1 "/mnt/c/Program\ Files/Python38/python.exe %:p" Enter<CR><CR>
" nmap <leader>ew :!tmux send-keys -t 1 "/mnt/c/Python38/python.exe %:p" Enter<CR><CR>
nmap <leader>ew :!tmux send-keys -t 1 "/mnt/c/Python38/python.exe %" Enter<CR><CR>
...
" nmap <leader>ea :!tmux send-keys -t 1 "arduino-cli compile --fqbn esp8266:esp8266:nodemcuv2 '%:p:h' -j 1 -v" Enter<CR><CR>
" --- for arduino nano
nmap <leader>ea :!tmux send-keys -t 1 "arduino-cli compile --fqbn arduino:avr:nano '%:p:h' -j 8 -v" Enter<CR><CR>
nmap <leader>es :!tmux send-keys -t 1 "arduino-cli compile --fqbn arduino:avr:nano '%:p:h' -j 8 --upload --port /dev/ttyS3 -v" Enter<CR><CR>
...
nmap <leader>ef :!tmux send-keys -t 1 "arduino-cli compile --fqbn arduino:avr:pro '%:p:h' -j 8 --upload --port /dev/ttyS3 -v" Enter<CR><CR>

" --- for esp8266
nmap <leader>ey :!tmux send-keys -t 1 "arduino-cli compile --fqbn esp8266:esp8266:nodemcuv2 '%:p:h' -j 8 --port /dev/ttyS3 -v" Enter<CR><CR>
"
...
nmap <leader>eu :!tmux send-keys -t 1 "arduino-cli compile --fqbn esp8266:esp8266:nodemcuv2 '%:p:h' --upload --port /dev/ttyS3 -v" Enter<CR><CR>
"
" dos 의 arduino-cli.exe를 이용해야 OTA(over-the-air) 업로드가
" 가능했습니다.wsl 내에서는 mdns-dicovery가 작동하지 않았습니다.
" 따라서 dos 식 파일명으로 정규식 교체 문법을 넣어줬습니다
" 또한 패스워드를 물어보는데 더미를 전송해주기도 합니다
" o는 over-the-air 의 o를 단축키로 지정하기로 했습니다
nmap <leader>eo :!tmux send-keys -t 1 "arduino-cli.exe compile --fqbn esp8266:esp8266:nodemcuv2 '%:p:s?mnt/./??' --upload --protocol network --port 192.168.1.72 -v --discovery-timeout 20s" Enter<CR><CR><CR>:!tmux send-keys -t1 "a" Enter<CR><CR>

" 특정 ip입력을 받아 업로드하는 단축키입니다
" ex)) https://stackoverflow.com/questions/57788828/vim-macro-which-accepts-user-input
function! UploadToIp()
  let newname = input('ESP Ip? ')
  execute "silent !tmux send-keys -t 1 'arduino-cli.exe compile --fqbn esp8266:esp8266:nodemcuv2 %:p:s?mnt/./?? --upload --protocol network --port 192.168.1.".newname." -v --discovery-timeout 20s' Enter \<cr>" 
  execute "redraw!"
  " 단축키 등록처럼 <CR>로 쉘을 닫는 방법을 찾을 수 없어 silent 와 redraw를
  " 통해서 해결하기로 합니다"
  "return '0/\<John\>'."\<cr>cw".newname."\<esc>n."
  " ex)) https://stackoverflow.com/questions/48977808/hide-the-output-of-shell-commands-in-vim
endfunction
nnoremap <expr> <leader>ei UploadToIp()
...
"
" for flutter
nmap <leader>wf :!tmux send-keys -t 1 "flutter run" Enter<CR> <CR>
"" hot reload
nmap <leader>wr :!tmux send-keys -t 1 "r" Enter<CR> <CR>    
"" restart
nmap <leader>wt :!tmux send-keys -t 1 "R" Enter<CR> <CR>
nmap <leader>wb :!tmux send-keys -t 1 "flutter build apk" Enter<CR> <CR>
...
" augroup CocSettingsJsonc
"   autocmd!
"   autocmd BufEnter coc-settings.json setlocal filetype=jsonc
" augroup END
